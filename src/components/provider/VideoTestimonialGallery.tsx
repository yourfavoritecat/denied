        ...
        <div className="relative aspect-[9/16] max-h-[85vh] flex items-center justify-center bg-black">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 pb-6">
            <div className="flex items-center gap-1 mb-1">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star key={s} className={`w-3.5 h-3.5 ${s <= current.review.rating ? "fill-primary text-primary" : "text-white/30"}`} />
              ))}
            </div>
...
const VideoTestimonialGallery = ({ reviews, compact = false }: VideoTestimonialGalleryProps) => {
  ...
  const thumbnails = videoReviews.map((vr, i) => (
    <button
      ...
      className="shrink-0 w-[100px] aspect-[9/16] rounded-lg overflow-hidden bg-black border border-border/50 hover:ring-2 hover:ring-primary transition-all relative group"
    >
      <video
        src={vr.videoUrl}
        className="w-full h-full object-cover"
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-bold flex items-center gap-2">
        <Play className="w-5 h-5 text-primary" /> Patient Videos
      </h3>
